...
namespace CoreRemoting.Tests
{
	public class NeoBinarySerializationTests
	{
		...
		public void NeoBinarySerializerAdapter_should_serialize_and_deserialize_collections()
		{
			...
			var deserializedCustomObservableCollection =
				serializer.Deserialize<ObservableCollection<TestComplexObject>>(serialized);
			...
			for (int i = 0; i < customObservableCollection.Count; i++)
			{
				...
				Assert.Equal(customObservableCollection[i].IsActive,
					deserializedCustomObservableCollection[i].IsActive);
			}
		}
		...
		public void NeoBinarySerializerAdapter_should_serialize_typed_DataSet_as_DiffGram()
		{
			...
			Assert.Equal(originalRow["UserName", DataRowVersion.Current],
				deserializedRow["UserName", DataRowVersion.Current]);
		}
		...
		[Fact]
		public void
			NeoBinarySerializerAdapter_should_serialize_and_deserialize_DataTable_with_UniqueConstraint_using_binary()
		{
			var config = new NeoBinarySerializerConfig
			{
				AllowUnknownTypes = true,
			...
			// Set primary key, which creates a UniqueConstraint automatically
			...
			var serialized = serializer.Serialize(table);
			var deserializedTable = serializer.Deserialize<DataTable>(serialized);
			...
			Assert.Equal("Id", deserializedUnique.Columns[0].ColumnName);
		}


		[Fact]
		public void
			NeoBinarySerializerAdapter_should_serialize_and_deserialize_DataSet_with_relations_and_constraints_using_binary()
		{
			var config = new NeoBinarySerializerConfig
			{
				AllowUnknownTypes = true,
			...
			// Parent table
			var parentTable = new DataTable("ParentTable");
			...
			// Add relation
			var relation = new DataRelation("ParentChild", parentTable.Columns["Id"], childTable.Columns["ParentId"]);
			...
			childRow["Id"] = 1;
			childRow["ParentId"] = 1;
			childRow["Description"] = "Child";
			childTable.Rows.Add(childRow);

			var serialized = serializer.Serialize(dataSet);
			var deserializedDataSet = serializer.Deserialize<DataSet>(serialized);
			...
			Assert.Equal(deserializedParent.Columns["Id"], deserializedRelation.ParentColumns[0]);
			Assert.Equal(deserializedChild.Columns["ParentId"], deserializedRelation.ChildColumns[0]);
		}
