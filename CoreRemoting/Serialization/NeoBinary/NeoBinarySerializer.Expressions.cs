		...
		Dictionary<object, int> objectMap)
	{
		...
		switch (expression.NodeType)
		{
			...
			case ExpressionType.Call:
				...
				foreach (var arg in callExpr.Arguments) SerializeExpression(arg, writer, serializedObjects, objectMap);
			...
			case ExpressionType.New:
				...
				foreach (var arg in newExpr.Arguments) SerializeExpression(arg, writer, serializedObjects, objectMap);
			...
			case ExpressionType.ListInit:
				...
				foreach (var init in listInitExpr.Initializers)
				{
					...
					foreach (var arg in init.Arguments) SerializeExpression(arg, writer, serializedObjects, objectMap);
				}
		...
		switch (nodeType)
		{
			...
			case ExpressionType.MemberAccess:
				...
				var maMemberInfo = maMemberExpr.Type.GetMember(maMemberName, maMemberType,
					...
					.FirstOrDefault();
				if (maMemberInfo != null) result = Expression.MakeMemberAccess(maMemberExpr, maMemberInfo);
			...
			case ExpressionType.NewArrayBounds:
				...
				for (var i = 0; i < arrayExprCount; i++)
					arrayExprs[i] =
						(Expression)DeserializeExpression(reader, deserializedObjects);
			...
			case ExpressionType.ListInit:
				...
				for (var i = 0; i < initCount; i++)
				{
					...
					for (var j = 0; j < liInitArgCount; j++)
