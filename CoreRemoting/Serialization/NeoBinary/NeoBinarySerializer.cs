...
public partial class NeoBinarySerializer
{
    ...
    private void WriteTypeInfo(BinaryWriter writer, Type type)
    {
        ...
        if (_currentContext?.TypeRefActive == true)
        {
            ...
            // Always use full type name with assembly info, but make version optional
    ...
    private Type ResolveTypeCore(string typeName, string assemblyName, string assemblyVersion)
    {
        ...
        {
            // The argDesc is the full assembly-qualified type name
            return ResolveTypeCore(argDesc, string.Empty, string.Empty);
        }

        // If the type name includes generic arguments ("[["), resolve recursively
        if (!string.IsNullOrEmpty(typeName) && typeName.Contains("[["))
        {
            // Extract generic definition part before first '['
            var genericDefEnd = typeName.IndexOf('[');
            var genericDefName = typeName.Substring(0, genericDefEnd);

            // Parse argument descriptors inside the outermost brackets
            var argsPart = typeName.Substring(genericDefEnd);
            var argDescriptors = ParseGenericArgumentNames(argsPart);
            var argTypes = new List<Type>();
            foreach (var desc in argDescriptors)
            {
                var resolved = ResolveArgument(desc);
                if (resolved == null)
                    throw new TypeLoadException($"Failed to resolve generic argument '{desc}' for type '{typeName}'.");
                argTypes.Add(resolved);
            }

            // Resolve the generic type definition itself
            var genericDefType = ResolveTypeCore(genericDefName, assemblyName, assemblyVersion);
            if (genericDefType == null)
                throw new TypeLoadException($"Failed to resolve generic type definition '{genericDefName}'.");

            // Construct closed generic type
            return genericDefType.MakeGenericType(argTypes.ToArray());
        }
        ...
        // Create cache key for type resolution - handle empty version properly
        var cacheKey = string.IsNullOrEmpty(assemblyVersion)
            ? $"{typeName}|{assemblyName}|no-version"
            : $"{typeName}|{assemblyName}|{assemblyVersion}";
        ...
        {
            // Special handling for anonymous types - they need exact type resolution
            if (typeName.StartsWith("<>f__AnonymousType"))
            {
                // For anonymous types, try exact match in loaded assemblies
                type = FindAnonymousTypeInLoadedAssemblies(typeName, assemblyName);
            }
            else
            {
                ...
                if (type == null)
                {
                    ...
                    // Fallback: Try Type.GetType with assembly context
                    if (type == null && string.IsNullOrEmpty(assemblyVersion))
                    {
                        // For test types, try assembly-qualified name
    ...
    /// <summary>
    /// Enhanced assembly-neutral type resolution with fallback mechanisms for test types.
    /// </summary>
    /// <param name="typeName">The assembly-neutral type name</param>
    /// <returns>Resolved type or null</returns>
    private Type ResolveAssemblyNeutralTypeWithFallback(string typeName)
    {
        // First try the normal resolution
        var type = ResolveAssemblyNeutralType(typeName);
        if (type != null) return type;

        // Extract generic argument types for special handling
        if (typeName.Contains("[["))
        {
            var argNames = ParseGenericArgumentNames(typeName.Substring(typeName.IndexOf("[[")));
            foreach (var argName in argNames)
            {
                // Try to resolve each argument type with enhanced fallback
                var argType = ResolveTypeWithEnhancedFallback(argName);
                if (argType == null) return null;
            }
        }

        // Retry the main resolution now that arguments should be cached
        return ResolveAssemblyNeutralType(typeName);
    }
    ...
    /// <returns>Resolved type or null</returns>
    private Type ResolveTypeWithEnhancedFallback(string typeName)
    {
        // Strategy 1: Normal assembly search
        var type = FindTypeInLoadedAssembliesCached(typeName);
        if (type != null) return type;
    ...
    private static List<string> ParseGenericArgumentNames(string argsPart)
    {
        ...
        while (i < argsPart.Length)
        {
            ...
            // Look for the specific separator pattern "],[" at depth 0
            // This handles both simple types and assembly-qualified names
            if (depth == 0 && i + 2 < argsPart.Length && 
                argsPart[i] == ']' && argsPart[i + 1] == ',' && argsPart[i + 2] == '[')
            {
                // Found separator, add current argument and start new one
    ...
    private string StripArgumentParameterSuffix(string message, string paramName)
    {
        ...
        return result.EndsWith(suffix2, StringComparison.Ordinal)
            ? result.Substring(0, result.Length - suffix2.Length)
            : result;
    }
