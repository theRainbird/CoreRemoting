...
namespace CoreRemoting.Serialization.NeoBinary
{
	...
	public static class NeoBinaryStreamAnalyzer
	{
		...
		private static void AnalyzeHeader(BinaryReader reader, StreamAnalysisReport report)
		{
			...
			if (reader.BaseStream.Position + 2 <= reader.BaseStream.Length)
			{
				...
				{
					report.CoreRemotingVersion = reader.ReadString();
				}
		...
		/// </summary>
		private static void AnalyzeObjectAtMarker(BinaryReader reader, byte marker, StreamAnalysisReport report, ref int depth)
		{
			switch (marker)
			{
				case 0: // Null marker
				...
				default:
					report.Errors.Add($"Unknown marker: {marker} (0x{marker:X2}) at position {reader.BaseStream.Position - 1}");
					report.HasErrors = true;
					break;
			}
		}
		...
		private static void DetectCorruptionPatterns(BinaryReader reader, StreamAnalysisReport report)
		{
			...
			var suspiciousBytes = new System.Collections.Generic.List<(long position, byte value)>();
	...
	public class StreamAnalysisReport
	{
		...
		public System.Collections.Generic.List<(long position, byte marker, string description)> ObjectMarkers { get; set; } = new();
		public System.Collections.Generic.List<int> ObjectIds { get; set; } = new();
		public System.Collections.Generic.List<int> ReferenceIds { get; set; } = new();
		...
		public System.Collections.Generic.List<(long position, byte value)> SuspiciousBytes { get; set; } = new();
