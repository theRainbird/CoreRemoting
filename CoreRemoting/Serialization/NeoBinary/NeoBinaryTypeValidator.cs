...
public class NeoBinaryTypeValidator
{
	...
	public void ValidateType(Type type)
	{
		...
		if (_allowedTypes.Count == 0 && _allowedNamespaces.Count == 0)
		{
			if (!AllowUnknownTypes)
				throw new NeoBinaryUnsafeDeserializationException(
					$"Unknown type '{type.FullName}' is not allowed. Set AllowUnknownTypes=true or add explicit allow rules.");
			return;
		}
	...
	public void ValidateExpression(System.Linq.Expressions.Expression expression)
	{
		if (!AllowExpressions)
			throw new NeoBinaryUnsafeDeserializationException(
				"Expressions are not allowed. Set AllowExpressions=true to enable.");
	...
	private void ValidateMemberAccess(System.Linq.Expressions.MemberExpression memberExpr)
	{
		...
		if (declaringType != null && IsDangerousType(declaringType))
			throw new NeoBinaryUnsafeDeserializationException(
				$"Member access to '{memberExpr.Member.Name}' on type '{declaringType.FullName}' is not allowed.");
	}
	...
	private IEnumerable<System.Linq.Expressions.Expression> GetChildExpressions(System.Linq.Expressions.Expression node)
	{
		switch (node.NodeType)
		{
