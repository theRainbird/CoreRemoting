...
Implement [IServerChannel](https://github.com/theRainbird/CoreRemoting/wiki/API-Reference#iserverchannel) and [IClientChannel](https://github.com/theRainbird/CoreRemoting/wiki/API-Reference#iclientchannel) to accomplish this _(Have a look at [Configuration](https://github.com/theRainbird/CoreRemoting/wiki/Configuration) to find out how you can tell CoreRemoting to use your custom channels)_.

## Message Encryption
If message encryption is enabled _(default setting)_, the serialized [messages](https://github.com/theRainbird/CoreRemoting/wiki/API-Reference#wiremessage) are signed and encrypted, before sent over the network.<br>
...
```
The authentication provider must be set to the AuthenticationProvider property of the [ServerConfig](https://github.com/theRainbird/CoreRemoting/wiki/Configuration#serverconfig).
...
--------|-----------|-------------
LinuxPamAuthProvider|Checks credentials against local Linux users<br>_(uses PAM)_<br>Credentials: **username**, **password**|https://www.nuget.org/packages/CoreRemoting.Authentication.LinuxPamAuthProvider/
WindowsAuthProvider|Checks credentials against Windows users<br>Credentials: **username**, **password**, _**domain**_ _(optional)_|https://www.nuget.org/packages/CoreRemoting.Authentication.WindowsAuthProvider/
GenericOsAuthProvider|Automatically detects OS _(Windows/Linux)_ and selects the proper authentication provider for authentication of local OS users<br>Credentials: **username**, **password**|https://www.nuget.org/packages/CoreRemoting.Authentication.GenericOsAuthProvider/
